...
from RequestUtils import RequestAdapter
...
class NEPSE(__Data):
    def __init__(self, stocks):
        ...
        self.request_client = RequestAdapter(headers=self.alt_headers, base_url=self.base_url)
        self.token_manager = TokenManager(self.request_client)
    ...
    def disable_tls_verification(self):
        self.tls_verification = False
    ...
    def get_indices(self):
        ...
        r_indices = self.request_client.get(self.url_index, headers=self.alt_headers).json()
        r_nepse = self.request_client.get(self.url_nepse, headers=self.alt_headers).json()

        self._stop_animation_thread()

        r_nepse = [x for x in r_nepse if x["index"] == "NEPSE Index"]
        r_indices.append(r_nepse[0])
        df = pd.DataFrame(columns=["index", "change", "perChange", "currentValue"])
        ...
        for i in r_indices:
            ...
            if i["change"] < 0:
                ...
                x["currentValue"] = f"{Style.RESET_ALL}" + str(i["currentValue"])
            if i["change"] > 0:
    ...
    def display_subindices(self):
        print(
            tabulate(
                self.subindices_df.sort_values("perChange", ascending=False),
                headers="keys",
                tablefmt="pretty",
                showindex=False,
            )
    ...
    def get_market_summary(self):
        ...
        x = self.request_client.get(self.url_market_summary, headers=self.get_authorization_headers()).json()
    ...
    def get_top_gainers(self):
        ...
        re = self.request_client.get(self.url_top_gainers, headers=self.get_authorization_headers()).json()
        ...
        self._print_table(
            df.filter(
                items=[
                    ...
                    "pointChange",
                    "percentageChange",
                    f"securityName{Style.RESET_ALL}",
                ]
            ).head(20)
        )
    ...
    def get_top_losers(self):
        ...
        re = self.request_client.get(self.url_top_losers, headers=self.get_authorization_headers()).json()

        self._stop_animation_thread()
        self.df_losers = pd.DataFrame(re)
        df = self.df_losers.rename(
            {
                "symbol": f"{Fore.RED}symbol",
                "securityName": f"securityName{Style.RESET_ALL}",
            },
            axis=1,
        )
    ...
    def get_top_turnover(self):
        ...
        re = self.request_client.get(self.url_top_turnover, headers=self.get_authorization_headers()).json()

        self._stop_animation_thread()
        self.df_turnover = pd.DataFrame(re)
        df = self.df_turnover.rename(
            {
                "symbol": f"{Fore.GREEN}symbol",
                "closingPrice": "LTP",
                "securityName": f"securityName{Style.RESET_ALL}",
            },
            axis=1,
    ...
    def get_top_volume(self):
        ...
        re = self.request_client.get(
            self.url_top_volume, headers=self.get_authorization_headers()
        ).json()
    ...
    def get_top_supplydemand(self, supply):
        response = self.request_client.get(
            self.url_top_supplydemand, headers=self.get_authorization_headers(), verify=False
        )
        re = response.json()
        if supply:
    ...
    def get_top_transactions(self):
        ...
        re = self.request_client.get(self.url_top_transactions, headers=self.get_authorization_headers()).json()
    ...
    def save_floorsheet(self, path):
        ...
        for _ in range(0, total_iteration):
            url_floorsheet = (
                "https://newweb.nepalstock.com.np/api/nots/"
            ...
            try:
                self._count = _ + 1
                r = requests.post(
                    url_floorsheet, headers=self.headers, data=data
                ).json()
            except:
                time.sleep(0.4)
                r = requests.post(
