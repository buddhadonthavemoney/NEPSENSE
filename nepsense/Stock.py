...
class Stock(__Data):
    def __init__(self, symbol):
        ...
        self.url_company = self.base_url + f"security/{self.c_id}"
        self.url_marketdepth = self.base_url + f"nepse-data/marketdepth/{self.c_id}"
        self.today_price = (
            self.base_url + "nepse-data/today-price?&size=500&businessDate=2021-06-23"
        )
    ...
    def get_basic_info(self):
        ...
        c_name = re.sub(
            r"[0-9]", "", self.df[condition].company_name.to_string()
        ).strip()
        c_sector = re.sub(
            r"[0-9]", "", self.df[condition].sector_name.to_string()
        ).strip()
    ...
    def get_price_info(self):
        ...
        try:
            ...
            x = requests.post(self.url_company, headers=self.headers, data=data).json()
        except Exception:
            time.sleep(0.4)
            self.headers["user-agent"] = self.get_user_agent()
            x = requests.post(self.url_company, headers=self.headers, data=data).json()
        ...
        try:
            ...
            print(
                (
                    f"{self.symbol} - {self.c_id} might be in merger or "
    ...
    def get_market_depth(self):
        ...
        print(tabulate(self.df_buy, headers="keys", tablefmt="pretty", showindex=False))
        total_buy = self.market_depth["totalBuyQty"]
        print(f"total_buy = {total_buy}")

        print(
            tabulate(self.df_sell, headers="keys", tablefmt="pretty", showindex=False)
        )
